...
/*---------------------------------------------------------------------------
1、名称：LCD_DisplayChinese_one
2、功能：在指定位置显示一个汉字
3、入口参数：x,y:起始坐标
            word:要显示的字符: -----暂时采用数字0,1来代替，，以后再优化
            size:字体的宽度 24(比较合适)
----------------------------------------------------------------------------*/
...
{  							  
	...
	if(size==24)  bytenum=72;        //从字库数组中可知道每种字体单个字符所占的字节数
	//else if(size==xx)bytenum=xx; //可以制作其他大小的字体
	else return;   //其他字体退出
    ...
    //cfont.h中字库
	 ...
	 {   
		//待优化************************************
		if(size==24)bytedata=char_24x24_chinese[word][b]; 	 	//调用24x24字体
		//else if(size==16)bytedata=char_1608[word][b];	//调用1608字体
		for(a=0;a<8;a++)   
		{	   
				if(bytedata&0x01) LCD_Color_DrawPoint(x,y,BRUSH_COLOR); //由于子模是低位在前 所以先从低位判断  为1时显示画笔颜色      
...
/*------------------------------------------------------------------------------
* 名    称: void LCD_DisplayChinese_string(u16 x,u16 y,u8 size,u8 *p)
* 功    能：显示汉字串
* 入口参数：x,y:起点坐标
*           size:字体大小
*           *p:汉字串起始地址	
